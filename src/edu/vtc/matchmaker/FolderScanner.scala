  ...
  private def processName(fullName: String) {
    ...
    for (name <- directoryContents) {
      ...
      if ( !honorExclusions || !(exclusions exists { _ == containedName }) ) {
        ...
        // Ignore hidden files and symbolic links. This ignores "dot" files on Unix and files with the hidden attribute
        // on Windows. Symbolic links should be copied when going Unix to Unix but they don't work going Unix to Windows.
        if (!containedFile.isHidden && !Files.isSymbolicLink(containedPath)) {
